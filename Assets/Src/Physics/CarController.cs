...
public class CarController : MonoBehaviour
{
    ...
    public bool simulationSteering = false;
	...
	void Start () 
	{
        ...
        Debug.Log(string.Format("CarController.cs -> Found {0} Input Devices!",Input.GetJoystickNames().Length));
        for(int i=0; i<Input.GetJoystickNames().Length; i++)
        {
            Debug.Log(string.Format("CarController.cs -> Device is ", Input.GetJoystickNames()[i]));
        }
	}
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.R))
        {
            ...
            for (int i = 0; i < rb.Length; i++)
            {
                ...
                if (rb[i].transform)
                {
                    ...
                    rb[i].rotation = Quaternion.Euler(rb[i].rotation.eulerAngles.x, rb[i].rotation.eulerAngles.y, 0);
                    rb[i].position = rb[i].transform.position + Vector3.up * 2;
                }
        ...
        // Toggle look back
        CarCamera.instance.lookBack = Input.GetKey(KeyCode.B) || Input.GetKey(KeyCode.Joystick1Button3);

        // Toggle view
        if (Input.GetKeyDown(KeyCode.V) || Input.GetKeyDown(KeyCode.Joystick1Button6))
        {
            // The camera position switcher is currently located at PlaceModels component
            PlaceModels pm = GetComponent<PlaceModels>();
            if (!pm) return;
            pm.NextCam();
        }
        ...
        // Joustick support!
        if (Input.GetJoystickNames().Length > 0 && !aiDrive)
        {
            horizontalInput = Input.GetAxis("Horizontal1");
            verticalInput = Input.GetAxis("Vertical1");
            // This should be used when we are planning to use smooth steering(for gamepads, not ideal for steering wheels)
            if(!simulationSteering) steerInput = Mathf.Sign(horizontalInput);
        }
        ...
        {
            if (simulationSteering)
                steering = horizontalInput;
            else
                steering *= Mathf.Abs(horizontalInput);

            throttle = Mathf.Clamp01(verticalInput);
            brake = Mathf.Clamp01(-verticalInput);
            ...
            throttleInput = Mathf.Clamp(throttle - brake, -1, 1);
            handbrake = Mathf.Clamp01((Input.GetKey(KeyCode.JoystickButton0) ? 1 : 0));
        }
        ...
        if (Input.GetKeyDown(KeyCode.F2)) drivetrain.automatic = !drivetrain.automatic;
        if (Input.GetKeyDown(KeyCode.F3)) tractionControl = !tractionControl;
        if (Input.GetKeyDown(KeyCode.F4)) simulationSteering = !simulationSteering;

        if (Input.GetKeyDown(KeyCode.A) || Input.GetKeyDown(KeyCode.JoystickButton1))
        {
