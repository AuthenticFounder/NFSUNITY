...
public class Wheel : MonoBehaviour
{
    ...
    public Skidmarks Skid { get { return skid; }  set { skid = value; } }
    ...
    Vector3 SuspensionForce()
    {
        ...
        springForce = Mathf.Clamp(springForce, 0, 30000);
        normalForce = springForce;

        float v = Vector3.Dot(localVelo, groundNormal);
        float damperForce = Mathf.Sign(v) * Mathf.Sqrt(Mathf.Abs(v)) * damping;

        return (springForce - damperForce + suspensionForceInput) * Vector3.up;
    }
    ...
    Vector3 RoadForce()
    {
        ...
        slip = Mathf.Sqrt(slipRatio * slipRatio + slipAngle * slipAngle);
    ...
    void FixedUpdate()
    {
        ...
        if (onGround)
        {
            ...
            roadForce = RoadForce();
            body.AddForceAtPosition(suspensionForce + roadForce, hit.point);
        }
        else
        {
            compression = 0.0f;
        ...
        if (skid != null && Mathf.Abs(slip) > 0.12f && onGround)
            lastSkid = skid.AddSkidMark(hit.point, hit.normal, Mathf.Abs(slip) - 0.12f, lastSkid);
        else
            lastSkid = -1;
        ...
        float steeringDampFactor = 0.1f;
        smoothSteer = Mathf.Lerp(smoothSteer, steeringAngle, Time.deltaTime / steeringDampFactor);
        steeringAngle = smoothSteer;
        if (model != null)
        {
            model.transform.localPosition = Vector3.up * (compression - 1.0f) * suspensionTravel;
            model.transform.localRotation = Quaternion.Euler(Mathf.Rad2Deg * rotation, steeringAngle, 0);
        }
        if (modelHub != null)
        {
            modelHub.transform.localPosition = Vector3.up * (compression - 1.0f) * suspensionTravel;
            modelHub.transform.localRotation = Quaternion.Euler(0, steeringAngle, 0);
        }
        if (modelSpeed != null)
        {
            bool speedToggle = Mathf.Abs(angularVelocity) > 30;
            model.gameObject.SetActive(!speedToggle);
            modelSpeed.gameObject.SetActive(speedToggle);
            modelSpeed.transform.localPosition = Vector3.up * (compression - 1.0f) * suspensionTravel;
            modelSpeed.transform.localRotation = Quaternion.Euler(Mathf.Rad2Deg * rotation, steeringAngle, 0);
        }
    }
    float smoothSteer;
}
