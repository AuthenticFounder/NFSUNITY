...
public class Drivetrain : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        float upShiftGripRatio = 0.8f;
        float downShiftGripRatio = 0.5f;
        if (automatic)
        {
            if (rpm >= maxRPM * (0.5f + 0.5f * throttleInput) && slipRatio < upShiftGripRatio)
                ShiftUp();
            else if (rpm <= maxRPM * (0.15f + 0.4f * throttleInput) && gear > 2 && slipRatio < downShiftGripRatio)
                ShiftDown();
    ...
    public void ShiftUp()
    {
        if (gear < gearRatios.Length - 1 && gear > 0)
            gear++;
    }
    ...
    public float Gear { get { return (gear - 1); } }

    public float Speed { get { return GetComponent<Rigidbody>().velocity.magnitude * 3.6f; } }
